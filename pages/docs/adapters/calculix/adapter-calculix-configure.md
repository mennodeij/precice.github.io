...
When using faces mesh, instead of a node set (\*NSET), a \*SURFACE must be sent, defined by a list of elements and face numbers. Instead of starting with a "N", the name must start with a "S".
...
The adapter supports modal dynamic simulations, where eigenmodes from a frequency analysis are used. Run the frequency analysis without preCICE (`ccx_preCICE -i [CalculiX input file]`) then run a coupled simulation. Currently, only explicit coupling is supported.
